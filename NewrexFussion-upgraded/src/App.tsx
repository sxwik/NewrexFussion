...
export default function App() {
  ...
  return (
    <div className="flex h-screen bg-[var(--bg-main)] text-[var(--text-primary)] font-sans antialiased overflow-hidden selection:bg-[var(--bg-inverse)]/20">
      ...
      {/* Sidebar */}
      <aside className="w-[260px] flex-shrink-0 flex flex-col bg-[var(--bg-main)] border-r border-[var(--border-light)] z-20">
        <div className="px-5 py-7 flex items-center gap-3">
          {/* Premium geometric logo mark */}
          <div className="w-7 h-7 flex-shrink-0">
            <svg viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
              <circle cx="10" cy="10" r="6.5" stroke="var(--accent-primary)" strokeWidth="1.2" strokeOpacity="0.7"/>
              <circle cx="18" cy="18" r="6.5" stroke="var(--accent-primary)" strokeWidth="1.2" strokeOpacity="0.35"/>
              <circle cx="14" cy="14" r="2.2" fill="var(--accent-primary)"/>
            </svg>
          </div>
          <h1 className="font-serif text-[22px] tracking-wide text-[var(--text-primary)] whitespace-nowrap overflow-hidden text-ellipsis flex items-center gap-2.5">
            {currentAgent === 'Fusion' ? 'Fusion' : `Fusion — ${currentAgent}`}
            <span className="text-[9px] accent-badge px-1.5 py-0.5 rounded-full tracking-widest uppercase font-sans font-medium flex-shrink-0">Beta</span>
          </h1>
        </div>
      ...
      {/* Main Content */}
      <div className="flex-1 relative flex flex-col min-w-0 bg-[var(--bg-main)] overflow-hidden grain-overlay">
        
        {/* Ambient Background — theme-aware */}
        <div className="absolute top-[-15%] left-[5%] w-[55vw] h-[55vw] bg-[radial-gradient(ellipse,rgba(70,100,220,0.065)_0%,transparent_70%)] blur-[120px] rounded-full pointer-events-none transition-all duration-1000"></div>
        <div className="absolute bottom-[-10%] right-[-5%] w-[45vw] h-[45vw] bg-[radial-gradient(ellipse,var(--accent-glow)_0%,transparent_70%)] blur-[120px] rounded-full pointer-events-none transition-all duration-1000"></div>
        
        {/* Top Header */}
        <header className="absolute top-0 right-0 left-0 px-6 py-5 flex justify-between items-center z-20 pointer-events-none">
          {/* Advanced Reasoning badge */}
          <div className="pointer-events-auto flex items-center gap-2 px-3 py-1.5 rounded-full border border-[var(--border-light)] bg-[var(--bg-overlay-02)] backdrop-blur-md">
            <span className="w-1.5 h-1.5 rounded-full bg-[var(--accent-primary)] shadow-[0_0_6px_var(--accent-primary)] animate-pulse"></span>
            <span className="font-mono text-[9px] tracking-[0.15em] uppercase text-[var(--accent-primary)]">Advanced Reasoning</span>
          </div>
          <div className="pointer-events-auto flex items-center gap-3">
            <button className="w-8 h-8 flex items-center justify-center hover:bg-[var(--bg-overlay-04)] rounded-full transition-colors text-[var(--text-muted)] hover:text-[var(--text-primary)]">
        ...
        <main className="flex-1 overflow-y-auto scroll-smooth flex flex-col items-center">
          ...
          {activeView === 'home' && messages.length === 0 && (
            ...
            >
              ...
              <div className="ambient-orb w-[500px] h-[320px] -top-24 left-1/2 -translate-x-1/2 bg-[radial-gradient(ellipse,rgba(70,100,220,0.07)_0%,transparent_70%)]" />
              <div className="ambient-orb w-[360px] h-[260px] bottom-12 left-[10%] bg-[radial-gradient(ellipse,var(--accent-glow)_0%,transparent_70%)]" />

              {/* Dot grid fade */}
              <div className="absolute inset-0 bg-dot-grid [mask-image:radial-gradient(ellipse_60%_55%_at_50%_50%,black_10%,transparent_80%)] opacity-50 pointer-events-none" />
              ...
              <motion.div 
                ...
                className="text-center mb-10 transform -translate-y-6 relative z-10"
              >
                <motion.p
                  initial={{ opacity: 0, y: 8 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.7, delay: 0.25 }}
                  className="font-mono text-[10px] tracking-[0.22em] uppercase text-[var(--text-muted)] mb-5"
                >
                  Cognitive infrastructure — v2.0 preview
                </motion.p>
                ...
                <motion.h2
                  ...
                  className="font-serif text-[72px] leading-none tracking-[-0.025em] mb-5 hero-title-shimmer"
                >
                  Fusion
                </motion.h2>
                ...
                <motion.p
                  ...
                  className="text-[15px] text-[var(--text-secondary)] tracking-[0.02em] font-light mb-6"
                >
                ...
                >
                  ...
                  <span className="feature-pill">Chain of Thought</span>
                  <span className="feature-pill">Multimodal</span>
                </motion.div>
              </motion.div>
              ...
              >
                <form 
                  ...
                  className="input-glass rounded-[28px] p-5 flex flex-col shadow-[0_8px_48px_rgba(0,0,0,0.45),0_0_0_1px_rgba(255,255,255,0.02)_inset]"
                >
                  <textarea
                    value={input}
                    onChange={(e) => {
                    ...
                    className="w-full py-2 px-2 min-h-[60px] max-h-[240px] text-[15px] focus:outline-none placeholder:text-[var(--text-muted-dark)] placeholder:italic text-[var(--text-primary)] bg-transparent resize-none leading-relaxed custom-scrollbar font-sans"
                    rows={2}
                  />
                  
                  <div className="flex justify-between items-center mt-3 pt-3 border-t border-[var(--border-light)]">
                    <div className="flex items-center gap-1.5">
                      <button type="button" className="w-9 h-9 flex items-center justify-center rounded-full bg-transparent text-[var(--text-muted-dark)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-overlay-03)] transition-all">
                        <Plus size={18} strokeWidth={1.5} />
                      </button>
                      <button 
                        type="button" 
                        onClick={() => setShowLiveChat(true)}
                        className="w-9 h-9 flex items-center justify-center rounded-full bg-transparent text-[var(--text-muted-dark)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-overlay-03)] transition-all"
                    ...
                    <div className="flex items-center gap-3">
                      <div className="hidden sm:flex items-center gap-1 font-mono text-[10px] text-[var(--text-muted-dark)]">
                        <span className="px-1.5 py-0.5 border border-[var(--border-light)] rounded text-[9px] bg-[var(--bg-overlay-02)]">⌘</span>
                        <span className="px-1.5 py-0.5 border border-[var(--border-light)] rounded text-[9px] bg-[var(--bg-overlay-02)]">↵</span>
                      </div>
                      ...
                      {isGenerating ? (
                        <motion.button
                          ...
                          className="w-9 h-9 rounded-full bg-[var(--bg-inverse)] text-[var(--bg-main)] flex items-center justify-center shrink-0 hover:bg-[var(--accent-hover)] transition-colors"
                        >
                          <StopCircle size={17} strokeWidth={2.5} fill="currentColor" />
                        </motion.button>
                      ) : (
                        <motion.button
                          ...
                          type="submit"
                          disabled={!input.trim()}
                          className="w-9 h-9 rounded-full bg-[var(--accent-primary)] text-[var(--bg-main)] flex items-center justify-center shrink-0 disabled:opacity-25 disabled:cursor-not-allowed send-btn-glow transition-all"
                        >
